	...
	return (table) => memo(() => [table.getState().grouping, table.getPreGroupedRowModel()], (grouping, rowModel) => {
		...
		const groupUpRecursively = function(rows, depth, parentId) {
			...
			return Array.from(rowGroupsMap.entries()).map((_ref, index) => {
				let [groupingValue, groupedRows$1] = _ref;
				let id = `${columnId}:${groupingValue}`;
				id = parentId ? `${parentId}>${id}` : id;
				const subRows = groupUpRecursively(groupedRows$1, depth + 1, id);
				subRows.forEach((subRow) => {
					subRow.parentId = id;
				});
				const leafRows = depth ? flattenBy(groupedRows$1, (row$1) => row$1.subRows) : groupedRows$1;
				...
				Object.assign(row, {
					...
					leafRows,
					getValue: (columnId$1) => {
						if (existingGrouping.includes(columnId$1)) {
							if (row._valuesCache.hasOwnProperty(columnId$1)) return row._valuesCache[columnId$1];
							if (groupedRows$1[0]) {
								var _groupedRows$0$getVal;
								row._valuesCache[columnId$1] = (_groupedRows$0$getVal = groupedRows$1[0].getValue(columnId$1)) != null ? _groupedRows$0$getVal : void 0;
							}
							return row._valuesCache[columnId$1];
						}
						if (row._groupingValuesCache.hasOwnProperty(columnId$1)) return row._groupingValuesCache[columnId$1];
						const column = table.getColumn(columnId$1);
						const aggregateFn = column == null ? void 0 : column.getAggregationFn();
						if (aggregateFn) {
							row._groupingValuesCache[columnId$1] = aggregateFn(columnId$1, leafRows, groupedRows$1);
							return row._groupingValuesCache[columnId$1];
						}
					}
				});
				subRows.forEach((subRow) => {
...
//#endregion
//#region node_modules/.pnpm/@tanstack+react-table@8.21.3_react-dom@19.2.3_react@19.2.3__react@19.2.3/node_modules/@tanstack/react-table/build/lib/index.mjs
/**
* react-table
*
* Copyright (c) TanStack
