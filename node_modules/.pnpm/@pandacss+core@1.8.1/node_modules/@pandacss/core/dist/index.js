...
var import_logger4, import_lightningcss, import_postcss5, import_browserslist, decoder;
var init_optimize_lightningcss = __esm({
  "src/plugins/optimize-lightningcss.ts"() {
    "use strict";
    import_logger4 = require("@pandacss/logger");
...
var Patterns = class {
  ...
  getPropertyType = (prop) => {
    switch (prop.type) {
      case "enum":
        return (0, import_shared12.unionType)(prop.value);
      case "token": {
...
var Recipes = class _Recipes {
  ...
  getConfigOrThrow = (0, import_shared14.memo)((name) => {
    ...
    if (!config) throw new import_shared14.PandaError("UNKNOWN_RECIPE", `Recipe "${name}" not found`);
    return config;
  });
...
var Stylesheet = class {
  ...
  process(options) {
    ...
    try {
      ...
      if (error instanceof import_postcss7.CssSyntaxError) {
        import_logger5.logger.error("sheet:process", error.showSourceCode(true));
      }
    }
    return;
  }
  ...
  toCss = ({ minify } = {}) => {
    try {
      ...
      if (error instanceof import_postcss7.CssSyntaxError) {
        import_logger5.logger.error("sheet:toCss", error.showSourceCode(true));
      }
      throw error;
    }
  };
...
var Context = class {
  constructor(conf) {
    ...
    this.hooks["context:created"]?.({ ctx: this.hooksApi, logger: import_logger6.logger });
  }
  studio;
  // Engines
  tokens;
