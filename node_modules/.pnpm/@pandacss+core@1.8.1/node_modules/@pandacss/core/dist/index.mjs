...
// src/conditions.ts
var order = ["at-rule", "self-nesting", "combinator-nesting", "parent-nesting", "mixed"];
...
var ImportMap = class {
  ...
  normalize = (map) => {
    if (isString2(map)) return this.fromString(map);
...
var Recipes = class _Recipes {
  ...
  inferJsxSlots = (name, recipe) => {
    ...
    if (_Recipes.isSlotRecipeConfig(recipe)) {
      const jsxRootName = capitalize5(ROOT_SLOT);
  ...
  getConfigOrThrow = memo4((name) => {
    ...
    if (!config) throw new PandaError3("UNKNOWN_RECIPE", `Recipe "${name}" not found`);
    return config;
  });
  find = memo4((jsxName) => {
...
var StaticCss = class {
  ...
  getCachedPropertyKeys = (property) => {
    ...
    if (!cached) {
      ...
      logger2.debug("static_css:wildcard", `${property} -> ${keys2.length} values (memoized)`);
    } else {
      logger2.debug("static_css:wildcard", `${property} (cache hit)`);
    }
  ...
  getCachedPatternPropertyValues = (patternName, property) => {
    ...
    if (!cached) {
      ...
      logger2.debug("static_css:wildcard", `Pattern ${patternName}.${property} -> ${values.length} values (memoized)`);
    } else {
      logger2.debug("static_css:wildcard", `Pattern ${patternName}.${property} (cache hit)`);
    }
  ...
  process = (options, stylesheet) => {
    ...
    const decoder2 = isClonedInstance ? context.decoder.clone() : this.decoder;
    const staticCss = {
      ...options,
    ...
    logger2.debug("static_css:process", `Processing staticCss`);
...
var Stylesheet = class {
  ...
  process(options) {
    ...
    try {
      ...
      if (error instanceof CssSyntaxError) {
        logger4.error("sheet:process", error.showSourceCode(true));
      }
    }
    return;
  }
   ...
   */
  processDecoderForRecipe = (decoder2, recipeName) => {
    const recipeSet = decoder2.recipes.get(recipeName);
    if (recipeSet) {
      recipeSet.forEach((recipe) => {
        this.processCss(recipe.result, recipe.entry.slot ? "recipes_slots" : "recipes");
      });
    }
    decoder2.recipes_base.forEach((baseSet, recipeKey) => {
  ...
  toCss = ({ minify } = {}) => {
    try {
      ...
      if (error instanceof CssSyntaxError) {
        logger4.error("sheet:toCss", error.showSourceCode(true));
      }
      throw error;
    }
  };
...
var Utility = class {
  ...
  assignDeprecated = (property, config) => {
    ...
    if (isString3(config.shorthand)) this.deprecated.add(config.shorthand);
...
var Context = class {
  constructor(conf) {
    ...
    this.hooks["context:created"]?.({ ctx: this.hooksApi, logger: logger5 });
  }
  studio;
  // Engines
  tokens;
